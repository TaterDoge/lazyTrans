...
import { SortableProviderItem } from "./components/sortable-provider-item";
...
export const TranslateSettings: VoidComponent<TranslateSettingsProps> = (
  ...
  const shouldShowModelConfig = () =>
    !!providerMeta()?.supportedModels?.length ||
    isCustomTranslateProvider(selectedProvider());

  const filteredProviderOrder = createMemo(() => {
    const keyword = providerSearch().trim().toLowerCase();
    if (!keyword) {
      return translateConfig.providerOrder;
    }

    return translateConfig.providerOrder.filter((providerId) => {
      const meta = getProviderMeta(providerId);
      return [providerId, meta?.name ?? "", meta?.description ?? ""].some(
        (value) => value.toLowerCase().includes(keyword)
      );
    });
  });

  const getProviderIndex = (providerId: TranslateProvider) =>
    translateConfig.providerOrder.indexOf(providerId);
  ...
  // const addCustomOpenAIProvider = () => {
  //   const customOpenAIProvider: TranslateProvider = `custom:${Date.now().toString(
  //     36
  //   )}-${Math.random().toString(36).slice(2, 8)}`;
  //
  //   translateActions.update({
  //     activeProvider: customOpenAIProvider,
  //     providers: [
  //       ...translateConfig.providers,
  //       getDefaultProviderConfig(customOpenAIProvider),
  //     ],
  //     providerOrder: [...translateConfig.providerOrder, customOpenAIProvider],
  //   });
  //   setSelectedProvider(customOpenAIProvider);
  // };
  ...
  return (
    <div class="flex h-full min-h-0 gap-6 overflow-hidden">
      <div class="flex min-h-0 w-52 shrink-0 flex-col gap-2 border-r">
        {props.serviceTabs}
        <ProviderSearchBox
                  ...
                  <Show when={shouldShowModelConfig()}>
                    <div class="flex items-center justify-between gap-4">
                      ...
                      <Show
                        ...
                        when={providerMeta()?.supportedModels?.length}
                      >
                        <Select
                          itemComponent={(props) => (
                          ...
                          options={
                            providerMeta()?.supportedModels?.map(
                              (m: string) => ({
                                value: m,
                                label: m,
                              })
